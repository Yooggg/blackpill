...
const char* http_explorer_page =
		...
		".action-btn{padding:3px 8px;background:#fff;border:1px solid #999;cursor:pointer;font-size:12px;margin-right:5px}"
		...
		".modal-content{background:white;padding:20px;border:1px solid #999;min-width:400px;max-width:80%;max-height:80%;overflow:auto}"
		...
		"#file-content{width:100%;min-height:400px;max-height:600px;overflow:auto;border:1px solid #ddd;padding:10px;background:#f9f9f9;font-family:monospace;font-size:12px;white-space:pre-wrap}"
		"#file-image{max-width:100%;height:auto}"
		".download-link{display:inline-block;padding:8px 15px;background:#0066cc;color:white;text-decoration:none;border-radius:3px;margin-top:10px}"
		...
		"<table id=\"file-table\">"
		"<thead><tr><th>Name</th><th>Type</th><th width=\"120\">Action</th></tr></thead>"
		...
		"<div class=\"modal-content\">"
		"<h3 id=\"view-filename\">File Viewer</h3>"
		"<div id=\"file-viewer\"></div>"
		"<button onclick=\"hideViewModal()\">Close</button>"
		"</div>"
		"</div>"
		...
		"<td>File</td>"
		"<td>"
		"<button class=\"action-btn\" onclick=\"viewFile('${path}${file}',event)\">View</button>"
		"<button class=\"delete-btn\" onclick=\"deleteItem('${path}${file}',event)\">Delete</button>"
		"</td>`;"
		"fileList.appendChild(row);"
		"});"
		...
		"if(!file) return;"
		"try{"
		"const response=await fetch(`/api/upload.cgi?path=${encodeURIComponent(currentPath)}&filename=${encodeURIComponent(file.name)}`,{"
		"method:'POST',"
		...
		"document.getElementById('view-filename').textContent='Viewing: '+filename;"
		"const viewer=document.getElementById('file-viewer');"
		"viewer.innerHTML='<div style=\"text-align:center;padding:20px\">Loading...</div>';"
		"document.getElementById('view-modal').classList.add('active');"
		"try{"
		"const response=await fetch(filepath);"
		"if(!response.ok){viewer.innerHTML='<div style=\"color:red\">Error loading file</div>';return;}"

		// Определяем тип файла
		"const isImage=['jpg','jpeg','png','gif','bmp','webp','svg'].includes(ext);"
		"const isText=['txt','log','json','xml','html','css','js','c','h','cpp','hpp','py','md','csv'].includes(ext);"

		"if(isImage){"
		// Изображение
		"const blob=await response.blob();"
		"const url=URL.createObjectURL(blob);"
		"viewer.innerHTML=`<img id=\"file-image\" src=\"${url}\" alt=\"${filename}\"><br>"
		"<a href=\"${filepath}\" download=\"${filename}\" class=\"download-link\">Download</a>`;"
		"}else if(isText){"
		// Текстовый файл
		"const text=await response.text();"
		"const escaped=text.replace(/</g,'&lt;').replace(/>/g,'&gt;');"
		"viewer.innerHTML=`<pre id=\"file-content\">${escaped}</pre>"
		"<a href=\"${filepath}\" download=\"${filename}\" class=\"download-link\">Download</a>`;"
		"}else{"
		// Бинарный файл - только скачивание
		"const size=response.headers.get('content-length')||'unknown';"
		"viewer.innerHTML=`<p>Binary file (${ext.toUpperCase()})</p>"
		"<p>Size: ${size} bytes</p>"
		"<a href=\"${filepath}\" download=\"${filename}\" class=\"download-link\">Download File</a>`;"
		"}"

		"}catch(error){"
		"viewer.innerHTML='<div style=\"color:red\">Error: '+error.message+'</div>';"
		"}"
		"}"
...
void Task_Web_Func()
{
	...
	while(1){
	    for(int i = 0; i < 4; i++) {
	        httpServer_run(http_server_socket_list[i]);
	    }
	}
